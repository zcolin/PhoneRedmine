...
public class WifiUtil
{
	...
	{
		String strAddr = null;
		WifiManager wifi = getWifiMgr(context);
		WifiInfo info = wifi.getConnectionInfo();
		if (info != null)
		{
			strAddr = info.getMacAddress();
		}

		/*如果wifi没有连接过，则读取的Mac地址会是null，这时读取文件可以获取*/
		if (strAddr == null)
		{
			strAddr = FileUtil.readFileStr("/sys/class/net/wlan0/address").substring(0, 17);
		}
		return strAddr == null ? "" : strAddr;
	}
	 ...
	 * @param Password 密码
	 */
	public void refreshWifi(Context context, String SSID, String Password)
	{
		WifiManager wifiManager = getWifiMgr(context);
		if (wifiManager != null)
		{
	...
	public static WifiInfo getWifiConnectInfo(Context context)
	{
		WifiManager wifiManager = getWifiMgr(context);
		WifiInfo wifiInfo = wifiManager.getConnectionInfo();
		if (!NetworkUtil.isWifiConnect(context) || wifiInfo == null || ("0x").equalsIgnoreCase(wifiInfo.getSSID())

...
import com.phone.redmine.acty.RMNewsViewActy;
import com.phone.redmine.adapter.RMNewsAdapter;
...
import com.phone.redmine.http.entity.RMNewsReply;
 ...
 * RM 新闻页面
 */
public class RMNewsFrag extends FragSupportLazyLoadBase implements OnItemClickListener
{

	private ArrayList<RMNews>	lstNews	= new ArrayList<RMNews>();
	private RMNewsAdapter		adapter;
	private FCZrcListView		lstView;
	private TextView			tvEmpty;
	private int					curPage;

	public static RMNewsFrag newInstance()
	{
		final RMNewsFrag newsFrag = new RMNewsFrag();
		return newsFrag;
	}
	...
	{
		if (position >= 0 && position < lstNews.size())
		{
			RMNews itemInfo = lstNews.get(position);
			if (itemInfo != null)
			{
				Intent intent = new Intent();
				intent.putExtra("newsId", itemInfo.id);
				intent.setClass(getActivity(), RMNewsViewActy.class);
	...
	private void notifyAdapter()
	{
		if (adapter == null)
		{
			...
			{
				adapter = new RMNewsAdapter(getActivity(), lstNews);
	...
	{
		StringBuilder url = new StringBuilder(RMHttpUrl.URL_NEWS);
			...
			public void success(Header[] headers, String response)
			{
				if (headers != null && response.length() > 0)
				{
					...
					try
					{
						...
						Gson gson = new Gson();
						RMNewsReply ps = gson.fromJson(response, RMNewsReply.class);
						if (ps != null && ps.news != null && ps.news.size() > 0)
						{
							lstNews.addAll(ps.news);
